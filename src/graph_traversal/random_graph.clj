...
(defn default-spanning-tree
  ...
  (let [vertices (map #(keyword (str %)) (range 1 (inc N)))] ; Vertices numbered from :1 to :N
    (map (fn [i]
           (let [start (nth vertices (dec i))
                 end (nth vertices i)
                 weight (if weighted
                          (+ weight-floor (rand-int (inc (- weight-cap weight-floor))))
                          1)] ; Random weight within limits
             (if weighted
...
(defn generate-random-edges
  ...
  (let [vertices (map #(keyword (str %)) (range 1 (inc N)))]
...
(defn add-edges-to-graph
  ...
  (reduce (fn [graph [start end & weight]]
            (update graph start (fnil conj [])
                    (if weighted [end (first weight)] end)))
          {}
          edges))
...
(defn ensure-all-vertices
  ...
  [graph N]
  (reduce (fn [g v]
            (let [vertex-key (keyword (str v))]
              (if (contains? g vertex-key)
                g
                (assoc g vertex-key []))))
          graph
          (range 1 (inc N))))
...
(defn make-graph
  ...
  Creates an unweighted graph if `weighted` is false. Throws an exception if `S` is less than N-1 or more than N(N-1).
  ...
  (let [min-edges (dec N)
    ...
    (when (or (< S min-edges) (> S max-edges))
      (throw (Exception.
              (str "Invalid number of edges. Must be between "
                   min-edges " and " max-edges ".")))))
  ...
  (let [spanning-tree (spanning-tree-fn N weighted weight-floor weight-cap)
        ...
        existing-edges (set spanning-tree)
        random-edges (generate-random-edges N remaining-edges weighted
                                            existing-edges weight-floor weight-cap)]
